...
public:
	...
	template<class T, int M, int N>
	void MAT_SUB(T A[M][N], T B[M][N], T C[M][N]) {
		for (int i = 0; i < M; i++) {
			for (int j = 0; j < N; j++) {
				C[i][j] = A[i][j] - B[i][j];
			}
	...
	template<class T1, class T2, class T3, int M, int N>
	void MAT_SUB(T1 A[M][N], T2 B[M][N], T3 C[M][N]) {
		for (int i = 0; i < M; i++) {
			for (int j = 0; j < N; j++) {
				C[i][j] = A[i][j] - B[i][j];
			}
...
//			   	std::cout << __FILE__ << __LINE__ << std::endl;
	...
	template<class T1, class T2, class T3, int M, int N>
	void MAT_VEC_MUL(T1 A[M][N], T2 B[N], T3 C[M]) {
		for (int i = 0; i < M; i++) {
			C[i] = 0;
			for (int j = 0; j < N; j++) {
				C[i] += A[i][j] * B[j];
	...
	template<class T, int M, int N>
	void VEC_MAT_MUL(T A[M], T B[M][N], T C[N]) {
		for (int i = 0; i < N; i++) {
			C[i] = 0;
			for (int j = 0; j < M; j++) {
				C[i] += A[j] * B[j][i];
	...
	template<class T1, class T2, class T3, int M, int N>
	void VEC_MAT_MUL(T1 A[M], T2 B[M][N], T3 C[N]) {
		for (int i = 0; i < N; i++) {
			C[i] = 0;
			for (int j = 0; j < M; j++) {
				C[i] += A[j] * B[j][i];
	...
	void MAT_MUL(T A[M][N], T B[N][P], T C[M][P]) {
		for (int i = 0; i < M; i++) {
			for (int j = 0; j < P; j++) {
				C[i][j] = 0;
				for (int k = 0; k < N; k++) {
	...
	void MAT_MUL(T1 A[M][N], T2 B[N][P], T3 C[M][P]) {
		for (int i = 0; i < M; i++) {
			for (int j = 0; j < P; j++) {
				C[i][j] = 0;
				for (int k = 0; k < N; k++) {
	...
	template<class T, int M>
	void SQUARE_MAT_MUL(T A[M][M], T B[M][M], T C[M][M]) {
		for (int i = 0; i < M; i++) {
			for (int j = 0; j < M; j++) {
				C[i][j] = 0;
				for (int k = 0; k < M; k++) {
	...
	template<class T1, class T2, class T3, int M>
	void SQUARE_MAT_MUL(T1 A[M][M], T2 B[M][M], T3 C[M][M]) {
		for (int i = 0; i < M; i++) {
			for (int j = 0; j < M; j++) {
				C[i][j] = 0;
				for (int k = 0; k < M; k++) {
	...
	void EXTEND_TO_SQUARE(T A[M][N], T B[S][S]) {
		for (int i = 0; i < S; i++) {
			for (int j = 0; j < S; j++) {
				if (i < M && j < N)
					B[i][j] = A[i][j];
	...
	void EXTEND_TO_SQUARE(T1 A[M][N], T2 B[S][S]) {
		for (int i = 0; i < S; i++) {
			for (int j = 0; j < S; j++) {
				if (i < M && j < N)
					B[i][j] = A[i][j];
	...
	template<class T, int M>
	void SQUARE_MAT_MUL_STRASSEN(int m, T A[M][M], T B[M][M], T C[M][M]) {
	...
	// QR Decomposition
	template<class T, int M, int N>
	void QRD_GS(T Mat[M][N], T MatQ[M][N], T MatR[N][N]) {
		// Initialisation
		int i = 0, j = 0, k = 0;
		for (i = 0; i < N; i++) {
			for (j = 0; j < M; j++) {
	...
	template<class T, int M, int N>
	void QRD_MGS(T Mat[M][N], T MatQ[M][N], T MatR[N][N]) {
		// Initialisation
		int i = 0, j = 0, k = 0;
		for (i = 0; i < N; i++) {
			for (j = 0; j < M; j++) {
	...
	template<class T, int M, int N>
	void QRD_HH(T Mat[M][N], T MatQ[M][N], T MatR[N][N]) {
		int i, j, k;
		//R=A;
		for (j = 0; j < M; j++) {
			for (i = 0; i < N; i++) {
	...
	template<class T, int M, int N>
	void QRD_HH(T **Mat, T **MatQ, T **MatR) {
		int i, j, k;
		//R=A;
		for (j = 0; j < M; j++) {
			for (i = 0; i < N; i++) {
...
//			std::cout << std::endl;
...

	{
		...
		const float x_scale = (In_rows > 1) ? (In_rows - 1.0f) / (Out_rows - 1.0f) : 0;
		const float y_scale = (In_cols > 1) ? (In_cols - 1.0f) / (Out_cols - 1.0f) : 0;
	...
	//using & to change value in vector
	void CONV_2D_NT( const Tri& MatA,const Convk& Kernel,const Vec& Bias,Tri& MatB,const bool padding = false,const int Stride = 1)
	{
		int Padding = padding ? (Kernel[0][0].size() - 1) / 2 : 0;
		int Out_rows = (MatA[0].size() + 2 * Padding - Kernel[0][0].size())/Stride +1;
		int Out_cols = (MatA[0][0].size() + 2 * Padding - Kernel[0][0].size())/Stride +1;
//		static_assert(Out_rows == (In_rows + 2 * Padding - Kernel_size)/Stride +1,"Conv output row dismatch!");
//		static_assert(Out_cols == (In_cols + 2 * Padding - Kernel_size)/Stride +1,"Conv output col dismatch!");
		...
		for (int c = 0;c < Kernal.size();c++)
		{
			for(int out_i=0;out_i < Out_rows;out_i += Stride)
			{
				for(int out_j =0;out_j <Out_cols;out_j += Stride)
				{
					...
					{
						for(int k_i = 0;k_i < Kernel[0][0].size();k_i += Stride)
						{
							for(int k_j =0;k_j <Kernel[0][0].size();k_j += Stride)
							{
								int in_i = out_i + k_i;
								int in_j = out_j + k_j;
								if (in_i < 0 || in_i >In_rows || in_j <0 ||in_j > In_cols)
								...
								{
									sum = sum + MatA[in_ch][in_i][in_j] * Kernel[c][in_ch][k_i][k_j];
								}
							}
						}
					}
	...
	//bilinear_interpolation without template
	//use large ap type to do with outrange data


	Mat B_INTERPLO_NT (const Mat& input,float scale = 1.0f)
	{
